...
TTAPI   NPRESULT TT_UpdateLatestFrame();              //== Process only the most recent =----
...
TTAPI   NPRESULT TT_SaveCalibrationW( const wchar_t *filename ); //== Save Calibration ===----
TTAPI   NPRESULT TT_SaveCalibration( const char *filename );
...
TTAPI bool      TT_CameraHasVisibleObjects(int cameraIndex);
...
TTAPI void      TT_AutoMaskAllCameras();
...
//== Start the camera calibration process by calling TT_StartCalibrationWanding().

TTAPI bool TT_StartCalibrationWanding();

enum TT_CameraCalibrationStates
{
    TT_CameraCalibration_Initialized = 0,
    TT_CameraCalibration_Sampling,
    TT_CameraCalibration_Solving,
    TT_CameraCalibration_Complete,
    TT_CameraCalibration_Uninitialized
};

//== Query the state of the camera calibration process with TT_CalibrationState().

TTAPI TT_CameraCalibrationStates TT_CalibrationState();

//== During calibration wanding TT_CameraCalibrationSamples() will return the current number
//== of wand samples for the given camera.

TTAPI int TT_CameraCalibrationSamples( int cameraIndex );

//== During calibration wanding TT_CalibrationSufficientSamples() will return true if enough
//== wand samples have been collected for calibration calculation to begin.

TTAPI bool TT_CalibrationSufficientSamples();

//== During calibration wanding, call TT_CameraWandLocation() with the camera index and it will
//== return true if the wand is present as well as an array of 6 float values.  The
//== 6 values are: Wand Centroid 1 x,y, Wand Centroid 2 x,y, Wand Centroid 3 x,y.

TTAPI bool TT_CameraWandLocation( int cameraIndex, float points[] );

//== Start the camera calibration calculation & solve by calling TT_CameraCalibrationSolve();

TTAPI bool TT_CameraCalibrationSolve();

//== Once the calibration state reaches TT_CameraCalibration_Complete, query the mean error (in meters)
//== and mean wand error (in meters).

TTAPI float TT_CalibrationResultMeanError();
TTAPI float TT_CalibrationResultMeanWandError();

//== Apply the resulting camera calibration calling TT_CameraCalibrationApplyResult().

TTAPI bool  TT_CameraCalibrationApplyResult();

//== To discard the camera calibration result, call TT_CameraCalibrationReset().

TTAPI bool  TT_CameraCalibrationReset();
...
class TTAPI cCameraCalibrationSettings
{
public:
    ...
    double WandLength;
    double WandCenterOffset;
    eCalibrationType CalibrationType;
};

//== Get Calibration Settings by calling TT_CalibrationSettings().

TTAPI NPRESULT TT_CalibrationSettings( cCameraCalibrationSettings& settings );

//== Set Calibration Settings by calling TT_SetCalibrationSettings().

TTAPI NPRESULT TT_SetCalibrationSettings( const cCameraCalibrationSettings& settings );
